...
public class CourseController {
    ...
    @ApiResponse(responseCode = "200", description = "성공적으로 코스 리스트를 불러왔습니다.",
            content = @Content(schema = @Schema(implementation = MyCourses.class)))
    public MyCourses getMyCourses() {
        MemberEntity memberEntity = memberService.getMemberEntity(jwtUtil.getMemberIdFromRequest());
        return courseService.getMyCourses(memberEntity);
    }
    ...
    @ApiResponse(responseCode = "200", description = "성공적으로 강의 코스를 등록하였습니다.",
            content = @Content(schema = @Schema(implementation = EnrolledCourseInfo.class)))
    public EnrolledCourseInfo enroll(@Valid @RequestBody NewLecture newLecture,
        ...
        if (ValidateUtil.checkIfPlaylist(newLecture.getLectureCode())) {
            ...
            return courseCreator.enroll(memberEntity, newLecture, useSchedule,
                    playlistService.getEnrollContentInfo(newLecture.getLectureCode()));
        } else {
            videoService.putVideo(videoService.getRecentVideo(newLecture.getLectureCode()));
            return courseCreator.enroll(memberEntity, newLecture, useSchedule,
                    videoService.getEnrollContentInfo(newLecture.getLectureCode()));
        }
    }
    ...
    @ApiResponse(responseCode = "200", description = "성공적으로 코스에 강의를 추가하였습니다.",
            content = @Content(schema = @Schema(implementation = EnrolledCourseInfo.class)))
    public EnrolledCourseInfo addLectureInCourse(@PathVariable("courseId") Long courseId,
                                                 @Valid @RequestBody NewLecture newLecture) {
        MemberEntity memberEntity = memberService.getMemberEntity(jwtUtil.getMemberIdFromRequest());

        if (ValidateUtil.checkIfPlaylist(newLecture.getLectureCode())) {
            playlistService.putPlaylistWithItemList(
                    playlistService.getRecentPlaylistWithItemList(newLecture.getLectureCode()));
            return courseService.addLecture(courseService.getCourseEntity(memberEntity, courseId),
                    playlistService.getEnrollContentInfo(newLecture.getLectureCode()));
        } else {
            videoService.putVideo(videoService.getRecentVideo(newLecture.getLectureCode()));
            return courseService.addLecture(courseService.getCourseEntity(memberEntity, courseId),
    ...
    @ApiResponse(responseCode = "200", description = "성공적으로 수강 정보를 반환하였습니다.",
    ...
    public CourseDetail getCourseDetail(@PathVariable(name = "courseId") Long courseId) {
        MemberEntity memberEntity = memberService.getMemberEntity(jwtUtil.getMemberIdFromRequest());
        return courseService.getCourseDetail(courseService.getCourseEntity(memberEntity, courseId));
    }

    @Auth
    @PutMapping("/lectures/{courseVideoId}/time")
    @Tag(name = "강의 수강")
    @Operation(summary = "시청중인 강의 학습시간 저장하기",
            description = "duration을 입력받아 업데이트하고, 70%가 넘었다면 수강완료 처리한다.")
    @ApiResponse(responseCode = "200", description = "성공적으로 학습시간을 저장 하였습니다.",
            content = {@Content(mediaType = "application/json", schema = @Schema(implementation = LectureStatus.class))})
    public LectureStatus updateLectureTime(@PathVariable(name = "courseVideoId") Long courseVideoId,
                                           @Valid @RequestBody LectureTimeRecord record,
                                           @RequestParam(name = "isCompletedManually", required = false,
                                                   defaultValue = "false") boolean isCompletedManually) {
        MemberEntity memberEntity = memberService.getMemberEntity(jwtUtil.getMemberIdFromRequest());
        CourseVideoEntity courseVideoEntity = courseService.getCourseVideoEntity(memberEntity, courseVideoId);
        return courseService.updateLectureTime(courseVideoEntity, record.getViewDuration(), isCompletedManually);
    }
    ...
    @ApiResponse(responseCode = "200", description = "강의코스 삭제가 성공했으며, 정상적으로 결과를 반환했습니다.",
            content = {@Content(mediaType = "application/json", schema = @Schema(implementation = MyCourses.class))})
    public MyCourses deleteCourse(@PathVariable(name = "courseId") Long courseId) {
        MemberEntity memberEntity = memberService.getMemberEntity(jwtUtil.getMemberIdFromRequest());
        courseRemover.remove(courseService.getCourseEntity(memberEntity, courseId));
        return courseService.getMyCourses(memberEntity);
    }
}
